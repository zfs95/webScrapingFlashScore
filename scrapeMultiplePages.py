...
driverFirst = webdriver.Firefox(executable_path="C:/Users/Florin's PC/Desktop/Web Scrappying/geckodriver.exe")
driverFirst.get("https://www.flashscore.com/football/england/premier-league/results/")
time.sleep(5)
firstContent = driverFirst.page_source
firstSoup = BeautifulSoup(firstContent, features="html.parser")
driverFirst.quit()
matchesIdClasses = []
matchesId=[]
for element in firstSoup.find_all(attrs="event__match"):
    name = element.get("id")
    if name not in matchesIdClasses:
        matchesIdClasses.append(name)

for item in matchesIdClasses:
    matchesId.append(item[4:])


for singleMatch in matchesId:
    driver = webdriver.Firefox(executable_path="C:/Users/Florin's PC/Desktop/Web Scrappying/geckodriver.exe")
    driver.get("https://www.flashscore.com/match/"+singleMatch+"/#match-summary/match-statistics/0")
    time.sleep(5)
    ...
    df.to_csv("leagueMatchesData.csv", mode="a",index=False, encoding='utf-8',errors="ignore",header=None)
